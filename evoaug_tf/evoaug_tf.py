...
class RobustModel_in_out(keras.Model):
    """Tensorflow keras.Model to specify how augmentation should be applied to a model
...
class RobustModel_per_seq(keras.Model):
    """Tensorflow keras.Model to specify how augmentation should be applied to a model
    ...
    def _apply_augment(self, x):
        ...
        # number of augmentations per sequence
        if self.hard_aug:
            batch_num_aug = self.max_augs_per_seq * tf.ones((tf.shape(x)[0],), dtype=tf.int32)
        else:
            batch_num_aug = tf.random.uniform(shape=(tf.shape(x)[0],), minval=1, maxval=self.max_augs_per_seq+1, dtype=tf.int32)

        i = tf.constant(0)
        x_new = tf.TensorArray(x.dtype, size=tf.shape(x)[0])
        while_condition = lambda i, _: tf.less(i, tf.shape(x)[0])

        def body(i, x_new):
            # randomly choose which subset of augmentations from augment_list
            aug_indices = tf.sort(tf.random.shuffle(tf.range(self.max_num_aug))[:batch_num_aug[i]])     
            seq = tf.expand_dims(x[i], axis=0)
            insert_status = True
            ...
            new_x = x_new.write(i, seq)
            new_i = i + 1
            return new_i, new_x

        _, x_new = tf.while_loop(while_condition, body, loop_vars=[i, x_new])
        return tf.squeeze(x_new.stack(), axis=1)
