...
group_name = "final_check"

filepath = '../datasets/deepstarr_data.h5'
x_train, y_train, x_valid, y_valid, x_test, y_test = H5DataLoader(filepath, downsample=20000)
_, L, A = x_valid.shape
...
for aug_index in range(len(augment_lists)):
    ...
    exp_name = exp_name_list[aug_index]
    
    trial_aug_results = []
    trial_finetune_results = []
    for trial in range(num_trials):
        exp_name = exp_name+"_"+str(trial)
        ...
        inputs, outputs = DeepSTARR_ori(input_shape=(L+evoaug_tf.augment_max_len(augment_list),A))#, tasks=['Dev','Hk'])
        model = rmodel.RobustModel(inputs=inputs, outputs=outputs, augment_list=augment_list, max_augs_per_seq=2, hard_aug=True)
        ...
        ckpt_path = os.path.join(output_dir, exp_name+"_aug.h5")
    ...
    with open(os.path.join(output_dir, exp_name+'_analysis.pickle'), 'wb') as fout:
